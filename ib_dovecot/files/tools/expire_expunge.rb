...
class DovecotExpire
  ...
  def conn
    @conn ||= begin
      ...
      require 'yaml'
      config = YAML.load_file(File.join(File.dirname(f),File.basename(f,'.rb')+'.config.yaml'))
      PGconn.connect(config['dbhost'], 5432, "","",config['dbname'], config['dbuser'],config['dbpwd'])
    end
  end
