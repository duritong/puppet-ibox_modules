...
-----END RSA PRIVATE KEY-----\n"
  ...
  describe 'normal operation' do
    before(:all) do
      ...
      File.open(File.join(@tmp_path,'test_1.key'),'w'){|f| f << @mx5guxonqb5cttqg_str }
      File.open(File.join(@tmp_path,'test_2.key'),'w'){|f| f << @mx5guxonqb5cttqg_str }
      File.open(File.join(@tmp_path,'test_3.key'),'w'){|f| f << @mx5guxonqb5cttqg_str }
      File.open(File.join(@tmp_path,'foo.key'),'w'){|f| f << @mx5guxonqb5cttqg_str }
      File.open(File.join(@tmp_path,'foo_1.key'),'w'){|f| f << @drpsyff5srkctr7h_str }
      File.open(File.join(@tmp_path,'bar_1.key'),'w'){|f| f << @drpsyff5srkctr7h_str }
    end
    after(:all) do
      FileUtils.rm_rf(@tmp_path) if File.exists?(@tmp_path)
    end
